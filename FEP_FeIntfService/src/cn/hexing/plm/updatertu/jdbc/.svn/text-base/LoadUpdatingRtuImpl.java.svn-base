package cn.hexing.plm.updatertu.jdbc;
...
import cn.hexing.plm.updatertu.LoadUpdatingRtuDao;
import cn.hexing.plm.updatertu.RtuStatus;
	...
	public List<RtuStatus> load() {
		...
		ParameterizedRowMapper<RtuStatus> rowMap = new ParameterizedRowMapper<RtuStatus>(){
			public RtuStatus mapRow(ResultSet rs, int rowNum) throws SQLException {
				int rtua = (int)Long.parseLong(rs.getString("ZDLJDZ").trim(),16);
