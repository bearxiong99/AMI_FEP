...
public class SyncTcpServer extends BaseSocketServer implements IModule {
	...
	private int pendingTime = 10; //用于csd，对于同一个手机号码，挂起时间
	...
	private void checkTimeout(){
		...
		for(SyncSocketClient client: list){
			if( now-client.getLastReadTime() > timeout*1000 ){
				log.warn("TCP Server["+port+"] force close client because of long time without communication,peerAddr:"+client.getPeerAddr());
...
//						}
		...
		private void doAccept()throws IOException{
			...
			SyncSocketClient client=null;
			if(clientClass==null){
				client = new SyncSocketClient(channel,SyncTcpServer.this);				
			}else{
				try{
					client=newInstanceClient(channel,SyncTcpServer.this);
				}catch(Exception e){
					client = new SyncSocketClient(channel,SyncTcpServer.this);				
				}
			}
		...
		private SyncSocketClient newInstanceClient(SocketChannel channel,
				SyncTcpServer syncTcpServer) throws SecurityException, NoSuchMethodException, ClassNotFoundException, IllegalArgumentException, InstantiationException, IllegalAccessException, InvocationTargetException {
			Class<SyncSocketClient> clazz = (Class<SyncSocketClient>) Class.forName(clientClass);
			Constructor<SyncSocketClient> constructor = clazz.getConstructor(SocketChannel.class,ISocketServer.class);
			SyncSocketClient ssc=constructor.newInstance(channel,SyncTcpServer.this);
			Field field =null;
			try {
				field = clazz.getField("pendingTime");
				field.setInt(ssc, pendingTime);
			} catch (Exception e) {}
			return ssc;
		}
	}
