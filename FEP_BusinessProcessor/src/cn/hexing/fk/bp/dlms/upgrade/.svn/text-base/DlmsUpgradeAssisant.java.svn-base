...
public class DlmsUpgradeAssisant {
	
	private static final TraceLog tracer = TraceLog.getTracer("DLMS.UPGRADE");
	private static final Logger log = Logger.getLogger(DlmsUpgradeAssisant.class);
	private static DlmsUpgradeAssisant instance;
	...
	public static DlmsUpgradeAssisant getInstance(){
		if(instance==null)
			instance= new DlmsUpgradeAssisant();
		return instance;
	}
	 ...
	 * @param status
	 */
	public void updateUpgradeInfo(DlmsRequest req,int status){
		UpgradeInfo info = new UpgradeInfo();
		info.setProtocol("03");
		info.setStatus(status);//设置升级状态
		RelayParam relayParam = req.getRelayParam();
		if(relayParam!=null){
			info.setLogicAddr(relayParam.getDcLogicalAddress());
			info.setTn(relayParam.getMeasurePoint());
		}else{
	 ...
	 * @param req
	 */
	public void onUpgradeSuccess(DlmsRequest req){
	 ...
	 * @return
	 */
	public byte[] getFtpFileContent(DlmsRequest req) {
	 ...
	 * @param req
	 */
	public void addRequestAppendParams(UpgradeInfo info, DlmsRequest req) {
		...
		if(effectDate!=null){
			...
			strDate =  DlmsConstant.getInstance().isIranTime?strDate=DateConvert.iranToGregorian(strDate):strDate;
		...
		if(info.getStatus()!=1 && info.getMaxSize()<=0) {
			if(tracer.isEnabled())
				tracer.trace("Upgrade Fail.Each frame max size must above 0,meterID:"+req.getMeterId());
			log.error("Upgrade Fail.Each frame max size must above 0,meterID:"+req.getMeterId());
		}
	...
	public void constructUpgradeInfo(DlmsRequest req, UpgradeInfo info) {
	 ...
	 * @param dlmsRequest
	 */
	public void onUpgradeFailed(DlmsRequest req) {
		try {
	 ...
	 * @param status
	 */
	public void updateUpgradeStatus(DlmsRequest req, String status) {
