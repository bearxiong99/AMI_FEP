...
/**
 * BCD码表示的时间和数值 YYMMDDHHmm和格式2
 *
 */
public class Parser52 {
	
	private static final Logger log = Logger.getLogger(Parser52.class);
	...
	public static Object parsevalue(byte[] data,int loc,int len,int fraction){
		...
		try{			
			...
			if(ok){
				...
				sb.append(":00");
				String value=ZjDateAssistant.upDateTimeProcess(sb.toString(), "yyyy-MM-dd HH:mm");
				sb = new StringBuffer();
				sb.append(value);
				sb.append(",");
	 ...
	 * 组帧
	 * @param frame 存放数据的帧
	 * @param value 数据值(前后不能有多余空字符 YYYY-MM-DD HH:MM,格式2)
	 * @param loc   存放开始位置
	 * @param len   组帧长度
	...
	public static int constructor(byte[] frame,String value,int loc,int len,int fraction){
		try{
			...
			for(int i=0;i<value.length();i++){
				...
				throw new MessageEncodeException("错误的 YYYY-MM-DD HH:mm 格式2 组帧参数:"+value);
			}
