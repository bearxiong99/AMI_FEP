		        ...
		        if(para.getRtuParams()!=null){
	        		for(FaalRequestRtuParam frp:para.getRtuParams()){			        	
			        	...
			        	for(int p=0;p<frp.getTn().length;p++){
					        ...
					        rowdata[1]=rights;
					        ZjDateAssistant.constructDateFrame(rowdata, 5, time); //这一句话替换下面5行
...
//					        rowdata[9]=ParseTool.IntToBcd(time.get(Calendar.MINUTE));	//minute
					        ...
					        rowdataHL[1]=rights;
					        ZjDateAssistant.constructDateFrame(rowdataHL, 5, time); //这一句话替换下面5行
...
//					        rowdataHL[9]=ParseTool.IntToBcd(time.get(Calendar.MINUTE));	//minute
					        ...
					        rowdataHLi[1]=rights;
					        ZjDateAssistant.constructDateFrame(rowdataHLi, 5, time); //这一句话替换下面5行
