        	...
        	if(obj instanceof FaalReadCurrentDataRequest){	//读取当前数据的请求对象
		        ...
		        if(para.getRtuParams()!=null){
	        		for(FaalRequestRtuParam frp:para.getRtuParams()){
				        ...
				        //组帧
			        	int len=0;//初始定义长度为0，根据不同的规约来确定长度
				        int loc=0;//读取数据项标示的位置初始定义为0，根据长度来确定读取位置
				        byte[] frame=new byte[len];	
			        	//判断帧类型：浙规len为8字节TNM+2*m 而广东规约只有2*m没有8个字节的TNM
			        	if(Protocol.GG.equals(para.getProtocol())){
					       len=datakeys.length*2;	//2*数据项个数
					        frame=new byte[len];
					        loc=0;
			        	}
			        	else if(Protocol.ZJ.equals(para.getProtocol())){
			        		    len=datakeys.length*2+8;	//8字节TNM+2*数据项个数
			        		    loc=8;
						        frame=new byte[len];
			        	...
			        	if(datamax>=(fdlen+8)||datakeys.length==1){//无须分帧
			        		...
			        		for(int j=0;j<ntnum;j++){			        			
			        			...
			        			if(Protocol.ZJ.equals(rtu.getRtuProtocol())){
				        			...
				        			if(curlen>datamax||j==ntnum-1){//数据				
					        			//组帧
				        				MessageZjHead head=createHead(rtu);			        						        						        		
