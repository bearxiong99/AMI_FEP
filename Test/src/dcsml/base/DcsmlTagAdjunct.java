 ...
 *
 */
public class DcsmlTagAdjunct implements ASN1Constants,Serializable{
	private static final long serialVersionUID = -8350387811160869562L;

	private static final Logger log = Logger.getLogger(DcsmlTagAdjunct.class);
	...
	public static final DcsmlTagAdjunct applicationImplicit(int num){
		return new DcsmlTagAdjunct(true,CLASS_APPLICATION ,num);
	}

	public static final DcsmlTagAdjunct applicationExplicit(int num){
		return new DcsmlTagAdjunct(false,CLASS_APPLICATION | PC_CONSTRUCTED ,num);
	}

	public static final DcsmlTagAdjunct contextSpecificImplicit(int num){
		return new DcsmlTagAdjunct(true,ASN1Constants.CLASS_CONTEXTSPECIFIC ,num);
	}

	public static final DcsmlTagAdjunct contextSpecificExplicit(int num){
		return new DcsmlTagAdjunct(false,ASN1Constants.CLASS_CONTEXTSPECIFIC | PC_CONSTRUCTED ,num);
	}

	public static final DcsmlTagAdjunct primitiveImplicit(int num){
		return new DcsmlTagAdjunct(true,ASN1Constants.CLASS_UNIVERSAL,num);
	}
	
	public static final DcsmlTagAdjunct primitiveExplicit(int num){
		return new DcsmlTagAdjunct(false,ASN1Constants.CLASS_UNIVERSAL | PC_CONSTRUCTED,num);
	}
	
	public DcsmlTagAdjunct(boolean isImplicit, int adjunctClass,int adjunctValue){
		implicit = isImplicit;
		tagClass = adjunctClass;
		tagValue = adjunctValue;
	}
	
	public void assign(DcsmlTagAdjunct adjunct){
	...
	public void assign(DcsmlType srcType){
		//axdrCodec = adjunct.axdrCodec;
	...
	public DcsmlTagAdjunct axdrCodec(boolean axdr){
		axdrCodec = axdr;
		return this;
	}
