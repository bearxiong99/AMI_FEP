...
import cn.hexing.tool.util.ToolConstant;
...
public class Decriptor extends javax.swing.JFrame {
    ...
    public enum MSG_MODE{UP,MASTER_DOWN,UPPER_DOWN};
    ...
    private static byte[] upperSystitle = new byte[]{0x48,0x45,0x43,0x00,0x05,0x00,0x00,0x01};
    ...
    private void initComponents() {
        ...
        c_msgType.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "上行", "主站下行", "上位机下行" }));
        ...
        t_encryptKey.setToolTipText("默认为32个零");
        ...
        t_authKey.setToolTipText("默认为32个零");
    ...
    private void b_createSystitleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_createSystitleActionPerformed
        ...
        try{
            if(checkAAREIslegal(strAare)){
            	if((strAare.substring(0, 2)).equals("00")){
            		strAare=strAare.substring(16);
            	}
                ...
                if(apTitle!=null){
                	...
                	ToolConstant.showMsg("解析出来的systitle为空");
                }
            }else{
            	ToolConstant.showMsg("不是正确的AARE");
            }
        }catch(Exception e){
            //exception
        	ToolConstant.showMsg("生成systitle出错");
        }
    }//GEN-LAST:event_b_createSystitleActionPerformed

    public boolean checkAAREIslegal(String aare){
        if(!ToolConstant.checkStringIsNotNull(aare)) return false;
        
        if("61".equals(aare.substring(0, 2)) || "61".equals(aare.substring(16, 18))) return true;
    ...
    private void c_msgTypeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c_msgTypeActionPerformed
        ...
        //index  1 = down
        //index  2 = 上位机下行
        msgMode=index==0?MSG_MODE.UP:(index==1?MSG_MODE.MASTER_DOWN:MSG_MODE.UPPER_DOWN);
        
        if(msgMode==MSG_MODE.MASTER_DOWN){
            systitle = masterSystitle;
            t_systitle.setText( HexDump.toHex(systitle));
        }else if(msgMode==MSG_MODE.UPPER_DOWN){
        	systitle = upperSystitle;
        	t_systitle.setText( HexDump.toHex(systitle));
        }
        else{
    ...
    private void b_encryptActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_encryptActionPerformed
        ...
        if(!ToolConstant.checkStringIsNotNull(encryText)){
        	ToolConstant.showMsg("要解密的帧为空!");
        	return ;
        }
        ...
        try {
			if(meterMode==METER_MODE.BENGAL){
				...
				if(ToolConstant.checkStringIsNotNull(strIv) ){
					plain=Gcm128SoftCipher.getInstance().decrypt(encryptKey, authKey, ciphered, HexDump.toArray(strIv));
				}else{
					ToolConstant.showMsg("iv 不能为空");
					return ;
				}
			}else{
				if(t_systitle.getText()!=null && !t_systitle.getText().trim().equals("")){
					if(!"00".equals(encryText.substring(0,2))){
						encryText="00010001000100"+Integer.toHexString(encryText.length()/2)+encryText;
					}
					IMessage msg = MultiProtoRecognizer.recognize(HexDump.toByteBuffer(encryText));
					if(msg==null || !(msg instanceof DlmsMessage)){
						ToolConstant.showMsg("不是正规的Dlms消息");
						return ;
					} 
					ASN1OctetString octs = new ASN1OctetString();
					msg.read(HexDump.toByteBuffer(encryText));
					ByteBuffer apdu = ((DlmsMessage)msg).getApdu();
					...
					ByteBuffer buffer = ByteBuffer.allocate(plain.length+8);
					...
					ToolConstant.showMsg("systitle 不能为空!");
					return;
				}
			}
			if(plain !=null){
